...
for(var i=orderedBlocks.length-1;i>=0;i--){if(orderedBlocks[i].start>row[k].stop){orderedBlocks[i].y=y;row.push.apply(row,orderedBlocks.splice(i,1));k++}}rows.push(row)}}/**
    ...
    */},{key:'_drawTrack',value:function _drawTrack(i){var _this3=this;var obj=this,//c = this._COLORS[i % this._COLORS.length],
t=this.data.tracks[i];this.viewer.append('defs').append('pattern').attr('id','diagonalHatch').attr('patternUnits','userSpaceOnUse').attr('width',4).attr('height',4).append('path').attr('d','M-1,1 l2,-2 M0,4 l4,-4 M3,5 l2,-2').attr('stroke','#000000').attr('stroke-width',1);// create the track's rows of blocks
...
var polygons=blocks.append('polygon').attr('class','block').style('fill',i%2?'#000000':'url(#diagonalHatch)').attr('points',function(b){var yTop=(obj._BLOCK_HEIGHT+obj._PAD)*b.y+obj._PAD,yBottom=yTop+obj._BLOCK_HEIGHT,yMiddle=yTop+obj._BLOCK_HEIGHT/2,block=d3.select(this);d3.select(this)// evil nested assignments!
...
return'translate('+x+', '+y+') rotate(-45)'})}.bind(this,polygons,tips);// how tips are adjusted so they don't overflow the view
