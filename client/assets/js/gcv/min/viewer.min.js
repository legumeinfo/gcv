...
clearTimeout(this._beginHoverTimeout);clearTimeout(this._endHoverTimeout);this._endHoverTimeout=setTimeout(function(){clearTimeout(this._endHoverTimeout);this._endHoverTimeout=undefined;// make sure nothing is being hovered
...
this._resize=this._resize.bind(this)}/**
    ...
    */},{key:'_drawTrack',value:function _drawTrack(i){var _this2=this;var obj=this,t=this.data.groups[i],y=this.ticks[i];// make svg group for the track
...
var transform=d3.transform(d3.select(this).attr('transform'));transform.translate=[x,y];return transform});if(track.highlight!==undefined){track.highlight.attr('width',this.viewer.attr('width'))}}.bind(this,geneGroups,lineGroups,lines,lineTips);// how tips are rotated so they don't overflow the view
...
o=tRect.bottom+h>vRect.bottom?h:0;return'translate('+o+', '+-o+') rotate(-45)'}).classed('synteny-tip',true);resize()}.bind(this,tips,track.resize);return track}/**
    ...
    */},{key:'_drawYAxis',value:function _drawYAxis(){var _this3=this;// construct the y-axes
var axis=d3.svg.axis().scale(this.y).orient('left').tickValues(this.ticks).tickFormat(function(y,i){return _this3.names[i]});// draw the axes of the graph
var yAxis=this.viewer.append('g').attr('class','axis').call(axis);yAxis.selectAll('text').attr('class',function(y,i){return i==0&&_this3.options.boldFirst?'query ':''}).attr('data-micro-track',function(y,i){return i.toString()}).attr('data-chromosome',function(y,i){return _this3.data.groups[i].chromosome_name}).style('cursor','pointer').on('mouseover',function(y,i){var iStr=i.toString(),micro='.GCV [data-micro-track="'+iStr+'"]',name=_this3.data.groups[i].chromosome_name,chromosome='.GCV [data-chromosome="'+name+'"]';var selection=d3.selectAll(micro+', '+chromosome).filter(function(){var t=this.getAttribute('data-micro-track');return t===null||t==iStr});_this3._beginHover(selection)}).on('mouseout',function(y,i){var iStr=i.toString(),micro='.GCV [data-micro-track="'+iStr+'"]',name=_this3.data.groups[i].chromosome_name,chromosome='.GCV [data-chromosome="'+name+'"]';var selection=d3.selectAll(micro+', '+chromosome).filter(function(){var t=this.getAttribute('data-micro-track');return t===null||t==iStr});_this3._endHover(selection)}).on('click',function(y,i){_this3.options.nameClick(_this3.data.groups[i])});return yAxis}/**
    * Creates the plot y-axis, placing labels at their respective locations.
    * @return {object} - D3 selection of the plot y-axis.
    */},{key:'_drawPlotAxis',value:function _drawPlotAxis(){var _this4=this;// construct the plot y-axes
var axis=d3.svg.axis().scale(this.y).orient('right').tickValues(this.ticks).tickFormat('plot');// draw the axes of the graph
var plotYAxis=this.viewer.append('g').attr('class','axis plot-axis').call(axis);plotYAxis.selectAll('text').style('cursor','pointer').on('click',function(y,i){_this4.options.plotClick(_this4.data.groups[i])});return plotYAxis}/** Draws the viewer. */},{key:'_draw',value:function _draw(){var _this5=this;// draw the y-axes
...
tracks.push(this._drawTrack(i).moveToBack())}// decorate the resize function with that of the track
...
if(this.options.autoResize){this.resizer=this._autoResize(this.container,function(e){_this5._resize()})}}// Public
/**
    * The constructor.
    * @param {HTMLElement|string} el - ID of or the element itself where the
    * viewer will be drawn in.
    ...
    * @param {object} data - The data the viewer will visualize.
    * @param {object} options - Optional parameters.
    */}]);function _class2(el,colors,data,options){_classCallCheck(this,_class2);this._PAD=5;this._GLYPH_SIZE=30;this._endHoverTimeout=0;this._init(el,colors,data,options);this._draw()}/** Makes a copy of the SVG and inlines external GCV styles. */_createClass(_class2,[{key:'_inlineCopy',value:function _inlineCopy(){// clone the current view node
