...
if(new.target===Graph.Node){throw new TypeError("Cannot construct Node instances directly")}this.attr=attr}return _class2}();Graph.Directed=(_temp=_class3=function(_Graph$Graph){_inherits(_class3,_Graph$Graph);function _class3(edgeDelimiter){_classCallCheck(this,_class3);return _possibleConstructorReturn(this,(_class3.__proto__||Object.getPrototypeOf(_class3)).call(this,edgeDelimiter))}_createClass(_class3,[{key:"removeNodeEdges",// primitive operations
value:function removeNodeEdges(id){var _this2=this;this.nodes[id].outNeighbors.forEach(function(v){_this2.removeEdge(id,v)});this.nodes[id].inNeighbors.forEach(function(v){_this2.removeEdge(v,id)})}},{key:"getEdgeId",value:function getEdgeId(u,v){return u+this.ed+v}},{key:"addEdge",value:function addEdge(u,v,attr){var e=this.getEdgeId(u,v);if(!this.edges.hasOwnProperty(e)){this.nodes[u].outNeighbors.add(v);this.nodes[v].inNeighbors.add(u);this.edges[e]=attr}}},{key:"updateEdge",value:function updateEdge(u,v,attr){var e=this.getEdgeId(u,v);if(this.edges.hasOwnProperty(e)){this.edges[e]=attr}}},{key:"removeEdge",value:function removeEdge(u,v){var e=this.getEdgeId(u,v);if(this.edges.hasOwnProperty(e)){delete this.edges[e];this.nodes[u].outNeighbors.delete(v);this.nodes[v].inNeighbors.delete(u)}}},{key:"getEdge",value:function getEdge(u,v){var e=this.getEdgeId(u,v);if(this.edges.hasOwnProperty(e)){return this.edges[e]}return null}}]);return _class3}(Graph.Graph),_class3.Node=function(_Graph$Node){_inherits(_class4,_Graph$Node);function _class4(attr){_classCallCheck(this,_class4);var _this3=_possibleConstructorReturn(this,(_class4.__proto__||Object.getPrototypeOf(_class4)).call(this,attr));_this3.inNeighbors=new Set;_this3.outNeighbors=new Set;return _this3}return _class4}(Graph.Node),_temp);Graph.Undirected=(_temp2=_class5=function(_Graph$Graph2){_inherits(_class5,_Graph$Graph2);function _class5(edgeDelimiter){_classCallCheck(this,_class5);return _possibleConstructorReturn(this,(_class5.__proto__||Object.getPrototypeOf(_class5)).call(this,edgeDelimiter))}_createClass(_class5,[{key:"removeNodeEdges",// primitive operations
...
return fr!==null?findFRs(fr,minsup,minsize,0):[]};Graph.MSAHMM=(_temp3=_class8=function(_Graph$Directed){_inherits(_class8,_Graph$Directed);function _class8(numColumns,characters,edgeDelimiter){_classCallCheck(this,_class8);var _this8=_possibleConstructorReturn(this,(_class8.__proto__||Object.getPrototypeOf(_class8)).call(this,edgeDelimiter));_this8.numColumns=numColumns;_this8.numCharacters=characters.size;_this8.baseProbability=1/(2+_this8.numCharacters);_this8.constructModel(characters);return _this8}_createClass(_class8,[{key:"addEdge",// primitive operations
value:function addEdge(u,v,attr){_get(_class8.prototype.__proto__||Object.getPrototypeOf(_class8.prototype),"addEdge",this).call(this,u,v,attr);this.nodes[u].attr.transitionCounts[v]=0}},{key:"removeEdge",value:function removeEdge(u,v){_get(_class8.prototype.__proto__||Object.getPrototypeOf(_class8.prototype),"removeEdge",this).call(this,u,v,attr);delete this.nodes[u].attr.transitionCounts[v]}// hmm operations
},{key:"indelTransitionProbability",value:function indelTransitionProbability(pathsOnState,pathsOnTransition){return this.baseProbability*(pathsOnState+1)/(pathsOnTransition+1)}},{key:"matchTransitionProbability",value:function matchTransitionProbability(pathsOnState,pathsOnTransition){return this.numCharacters*this.indelTransitionProbability(pathsOnTransition,pathsOnState)}},{key:"constructModel",value:function constructModel(characters){// add nodes
this.addNode("s",new Graph.MSAHMM.State);for(var i=0;i<this.numColumns;i++){var id="m"+i,m=new Graph.MSAHMM.MatchState(characters);this.addNode(id,m);id="i"+i;this.addNode(id,new Graph.MSAHMM.InsertState);id="d"+i;this.addNode(id,new Graph.MSAHMM.State)}this.addNode("i"+this.numColumns,new Graph.MSAHMM.InsertState);this.addNode("e",new Graph.MSAHMM.State);// add edges
this.addEdge("s","m0");this.addEdge("s","i0");this.addEdge("s","d0");for(var i=0;i<this.numColumns-1;i++){this.addEdge("m"+i,"m"+(i+1));this.addEdge("m"+i,"i"+(i+1));this.addEdge("m"+i,"d"+(i+1));this.addEdge("i"+i,"i"+i);this.addEdge("i"+i,"m"+i);this.addEdge("i"+i,"d"+i);this.addEdge("d"+i,"d"+(i+1));this.addEdge("d"+i,"m"+(i+1));this.addEdge("d"+i,"i"+(i+1))}this.addEdge("m"+(this.numColumns-1),"i"+this.numColumns);this.addEdge("m"+(this.numColumns-1),"e");this.addEdge("i"+this.numColumns,"i"+this.numColumns);this.addEdge("i"+this.numColumns,"e");this.addEdge("d"+(this.numColumns-1),"i"+this.numColumns);this.addEdge("d"+(this.numColumns-1),"e");this.updateTransitionProbabilities()}},{key:"updateTransitionProbabilities",value:function updateTransitionProbabilities(){for(var id in this.nodes){if(this.nodes.hasOwnProperty(id)){this.updateNodeTransitionProbabilities(id)}}}},{key:"updateNodeTransitionProbabilities",value:function updateNodeTransitionProbabilities(id){var pathsOnState=0;for(var pId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(pId)){pathsOnState+=this.nodes[id].attr.transitionCounts[pId]}}if(id=="d"+(this.numColumns-1)||id=="m"+(this.numColumns-1)||id=="i"+this.numColumns){for(var nId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(nId)){var pathsOnTransition=this.nodes[id].attr.transitionCounts[nId];var p=this.indelTransitionProbability(pathsOnState,pathsOnTransition);if(nId.startsWith("s")){p*=2}this.updateEdge(id,nId,p)}}}else{for(var nId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(nId)){var pathsOnTransition=this.nodes[id].attr.transitionCounts[nId];var p;if(nId.startsWith("m")){p=this.matchTransitionProbability(pathsOnState,pathsOnTransition)}else{p=this.indelTransitionProbability(pathsOnState,pathsOnTransition)}this.updateEdge(id,nId,p)}}}}}]);return _class8}(Graph.Directed),_class8.State=function(){function _class9(){_classCallCheck(this,_class9);this.transitionCounts={}}//addPath(pId, n) {
//  if (!this.paths.hasOwnProperty(pId)) {
//    this.paths[pId] = [];
//  }
//  this.paths[pId].push(n);
//}
return _class9}(),_temp3);Graph.MSAHMM.InsertState=function(_Graph$MSAHMM$State){_inherits(_class10,_Graph$MSAHMM$State);function _class10(){_classCallCheck(this,_class10);return _possibleConstructorReturn(this,(_class10.__proto__||Object.getPrototypeOf(_class10)).call(this));//this.insertions = {};
}//addPath(pId, o) {
//  super.addPath(pId);
...
//}
return _class10}(Graph.MSAHMM.State);Graph.MSAHMM.MatchState=function(_Graph$MSAHMM$State2){_inherits(_class11,_Graph$MSAHMM$State2);function _class11(characters){_classCallCheck(this,_class11);var _this10=_possibleConstructorReturn(this,(_class11.__proto__||Object.getPrototypeOf(_class11)).call(this));_this10.emissionCounts={};_this10.emissionProbabilities={};_this10.pseudoCount=characters.size;var p=1/_this10.pseudoCount;//this.numObservations = characters.size;
characters.forEach(function(o){_this10.emissionCounts[o]=1;// pseudo-count
_this10.emissionProbabilities[o]=p});return _this10}_createClass(_class11,[{key:"emit",value:function emit(o){return this.emissionProbabilities[o]}//addPath(pId, o) {
//  super.addPath(pId);
//  this.counts[o]       += 1;
...
//  }
//}
}]);return _class11}(Graph.MSAHMM.State);Graph.MSAHMM.viterbi=function(hmm,seq){var probs={},ptrs={};for(var id in hmm.nodes){if(hmm.nodes.hasOwnProperty(id)){probs[id]={};ptrs[id]={}}}// a generic probability forward propagate function
var propagate=function propagate(from,to,i){var current=probs[to][i]||-Infinity,candidate=probs[from][i-!to.startsWith("d")]+// arithmetic HACK!
Math.log(hmm.getEdge(from,to));if(to.startsWith("m")){candidate+=hmm.getNode(to).attr.emit(seq[i])}// TODO: use preferred transitions to resolve ties
if(candidate>current||candidate==current&&from.startsWith("m")){probs[to][i]=candidate;ptrs[to][i]=from}};// recursively identifies the sequence's most probable path through the HMM
var traceback=function traceback(id,i){if(id=="s"){return[id]}var ptr=ptrs[id][i],path=traceback(ptrs,i-!ptr.startsWith("d"));// arithmetic HACK!
path.push(id);return path};// seed start state
var s="s";probs[s][-1]=0;// = log(1)
...
if(i>0){propagate(ij,ij,i);propagate(ij,mj,i)}propagate(ij,dj,i);if(j<hmm.numColumns-1){djnext="d"+(j+1);mjnext="m"+(j+1);// delete and merge transitions out of deletion j
propagate(dj,djnext,i);propagate(dj,mjnext,i);// delete and merge transitions out of match j
propagate(mj,djnext,i);propagate(mj,mjnext,i)}ijnext="i"+(j+1);// insertion transition out of delete j
...
var e="e";propagate(dj,e,seq.length);propagate(ij,e,seq.length);propagate(mj,e,seq.length);// follow the pointers from the end state to the start state to get the path
return traceback(e,seq.length)};/**
  ...
  */Graph.msa=function(groups){var embedAlignmentPath=function embedAlignmentPath(hmm,pId,states,observations){//for (var i = 0; i < states.length; i++) {
...
//  }
//}
};var performSurgery=function performSurgery(hmm){};// 1) construct the graph from the first track
var l=groups[0].genes.length,families=new Set;for(var i=0;i<groups.length;i++){for(var j=0;j<groups[i].genes.length;j++){families.add(groups[i].genes[j].family)}}var hmm=new Graph.MSAHMM(l,families);console.log(hmm);// 2) iteratively add each remaining track to the alignment
for(var i=1;i<groups.length;i++){// a) align to HMM
var seq=groups[i].genes.map(function(g){return g.family});Graph.MSAHMM.viterbi(hmm,seq);// b) update the transition and emission probabilities using the new alignment
