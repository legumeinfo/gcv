...
var l=hmm.numColumns;for(var k=l+growBy-1;k>=j;k--){var knext=k+1,dk="d"+k,ik="i"+knext,mk="m"+k,dknext="d"+knext,mknext="m"+knext;// add new nodes and edges
if(k>=l-1){// add a new column
if(k>=l){hmm.addNode(dk);hmm.addNode(ik);hmm.addNode(mk);hmm.addEdge(dk,ik);hmm.addEdge(ik,ik);hmm.addEdge(mk,ik)}// connect this and the next column
if(k==l+growBy-1){hmm.addEdge(dk,"z");hmm.addEdge(ik,"z");hmm.addEdge(mk,"z")}else{hmm.addEdge(dk,dknext);hmm.addEdge(dk,mknext);hmm.addEdge(ik,dknext);hmm.addEdge(ik,mknext);hmm.addEdge(mk,dknext);hmm.addEdge(mk,mknext)}}// shift the existing probabilities
...
for(var k=Math.max(0,j-1);k<j+growBy;k++){var d="d"+k,i="i"+(k+1),m="m"+k;hmm.updateNodeTransitionProbabilities(d);hmm.updateNodeTransitionProbabilities(i);hmm.updateNodeTransitionProbabilities(m)}hmm.numColumns+=growBy}}};/**
