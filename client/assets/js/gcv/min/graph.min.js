...
for(var i=0;i<groups.length;i++){var seq1=groups[i].genes.map(function(g){return g.family}),path1=Graph.MSAHMM.viterbi(hmm,seq1),seq2=seq1.slice().reverse(),path2=Graph.MSAHMM.viterbi(hmm,seq2);if(path1.probability>=path2.probability){align(path1,groups[i].genes)}else{groups[i].genes.reverse();groups[i].genes.map(function(g){g.strand*=-1});align(path2,groups[i].genes)}}return groups};
