...
if(j==0){hmm.updateNodeTransitionProbabilities("a")}for(var k=Math.max(0,j-1);k<j+growBy;k++){var dk="d"+k,ik="i"+(k+1),mk="m"+k;hmm.updateNodeTransitionProbabilities(dk);hmm.updateNodeTransitionProbabilities(ik);hmm.updateNodeTransitionProbabilities(mk)}}}};/**
  ...
  */Graph.msa=function(tracks){var groups=JSON.parse(JSON.stringify(tracks)),counts=getFamilySizeMap({groups:groups});for(var i=0;i<groups.length;i++){groups[i].genes=groups[i].genes.filter(function(g){return g.family!=""&&counts[g.family]>1})}var rawGroups=JSON.parse(JSON.stringify(tracks));var align=function align(path,genes){var x=0,j=0,insertionSize=0;for(var i=1;i<path.length;i++){var n=path[i];if(n.startsWith("m")||n.startsWith("d")||n=="z"){if(insertionSize>0){var step=1/(insertionSize+1);for(var k=insertionSize;k>0;k--){genes[j++].x=x-k*step}insertionSize=0}if(n.startsWith("m")){genes[j++].x=x}x++}else{insertionSize++}}};// 1) construct a HMM with a column for each gene in the first track
...
for(var i=0;i<rawGroups.length;i++){var seq1=rawGroups[i].genes.map(function(g){return g.family}),path1=Graph.MSAHMM.viterbi(hmm,seq1),seq2=seq1.slice().reverse(),path2=Graph.MSAHMM.viterbi(hmm,seq2);if(path1.probability>=path2.probability){align(path1,rawGroups[i].genes)}else{rawGroups[i].genes.reverse();rawGroups[i].genes.map(function(g){g.strand*=-1});align(path2,rawGroups[i].genes)}}return rawGroups};
