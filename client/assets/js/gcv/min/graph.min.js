...
if(j==0){hmm.updateNodeTransitionProbabilities("a")}for(var k=Math.max(0,j-1);k<j+growBy;k++){var dk="d"+k,ik="i"+(k+1),mk="m"+k;hmm.updateNodeTransitionProbabilities(dk);hmm.updateNodeTransitionProbabilities(ik);hmm.updateNodeTransitionProbabilities(mk)}}}};/**
  ...
  * seq.
  */Graph.MSAHMM.sequenceEmissions=function(hmm,/*counts,*/seq,path){var eseq=[],j=0;for(var i=1;i<path.length;i++){var n=path[i];if(n.startsWith("m")/*&& counts[seq[j]] > 1*/){eseq.push(hmm.getNode(n).attr.emit(seq[j++]))}else if(n.startsWith("i")){eseq.push(0);j++}}return eseq};/**
  ...
  */Graph.MSAHMM.alignToPath=function(genes,path){var x=0,j=0,insertionSize=0;for(var i=1;i<path.length;i++){var n=path[i];if(n.startsWith("m")||n.startsWith("d")||n=="z"){if(insertionSize>0){var step=1/(insertionSize+1);for(var k=insertionSize;k>0;k--){genes[j++].x=x-k*step}insertionSize=0}if(n.startsWith("m")){genes[j++].x=x}x++}else{insertionSize++}}};/**
  ...
  */Graph.MSAHMM.orientationSequence=function(eseq1,eseq2){// determine if each character was better aligned in the forward (f) or
// reverse alignment (r), or if they effectively tied (t)
var merged=[],rlocs=[],fcounts=[0];for(var i=0;i<eseq1.length;i++){var p1=eseq1[i],p2=eseq2[eseq2.length-(i+1)];if(p1>p2/*&& p2 > 0*/){merged.push("f");fcounts[rlocs.length]+=1}else if(p1<p2){merged.push("r");rlocs.push(i);fcounts.push(0)}else{merged.push("t")}}rlocs.push(eseq1.length);fcounts.push(0);// resolve ties and flip characters that fracture larger blocks
for(var i=0;i<rlocs.length-1;i++){var l=rlocs[i];// convert t chains between r's with one or less f's to r's
if(fcounts[i+1]<=1){for(var j=l+1;j<rlocs[i+1];j++){merged[j]="r"}// flip island r's
}else{if(l-1>=0&&merged[l-1]!="r"){merged[l]="f"}for(var j=l+1;j<rlocs[i+1];j++){merged[j]="f"}}}// get the edge case - there's an inversion at the beginning
if(merged[rlocs[0]]=="r"&&fcounts[0]<=1){for(var j=0;j<rlocs[0];j++){merged[j]="r"}}else{for(var j=0;j<rlocs[0];j++){merged[j]="f"}}return merged};/**
  ...
  */Graph.MSAHMM.orientatedGenes=function(genes1,genes2,oseq){var genes=[];for(var i=0;i<oseq.length;i++){if(oseq[i]=="f"){genes.push(genes1[i])}else if(oseq[i]=="r"){var g=genes2[oseq.length-(i+1)];g.y=1;genes.push(g)}}return genes};/**
  * An HMM based MSA algorithm.
  * @param {Array} tracks - groups attribute of GCV track data.
  * @return {int} - The computed score.
  */Graph.msa=function(tracks){var groups=JSON.parse(JSON.stringify(tracks)),counts=getFamilySizeMap({groups:groups});for(var i=0;i<groups.length;i++){groups[i].genes=groups[i].genes.filter(function(g){return g.family!=""&&counts[g.family]>1})}var rawGroups=JSON.parse(JSON.stringify(tracks));// 1) construct a HMM with a column for each gene in the first track
var l=groups[0].genes.length,families=new Set;for(var i=0;i<groups.length;i++){for(var j=0;j<groups[i].genes.length;j++){var f=groups[i].genes[j].family;families.add("+"+f);families.add("-"+f)}}var hmm=new Graph.MSAHMM(l,families);// 2) iteratively train the HMM on the filtered tracks
for(var i=0;i<groups.length;i++){// a) align to HMM
var seq1=groups[i].genes.map(function(g){return(g.strand==-1?"-":"+")+g.family}),path1=Graph.MSAHMM.viterbi(hmm,seq1),seq2=groups[i].genes.slice().reverse().map(function(g){return(g.strand==-1?"+":"-")+g.family}),path2=Graph.MSAHMM.viterbi(hmm,seq2);// b) embed alignment path and update transition and emission probabilities
if(path1.probability>=path2.probability){Graph.MSAHMM.embedPath(hmm,i,path1,seq1)}else{Graph.MSAHMM.embedPath(hmm,i,path2,seq2)}// c) if necessary, perform surgery on the graph
Graph.MSAHMM.performSurgery(hmm)}// align the unfiltered tracks to the trained HMM
for(var i=0;i<rawGroups.length;i++){var genes1=JSON.parse(JSON.stringify(rawGroups[i].genes)),seq1=genes1.map(function(g){return(g.strand==-1?"-":"+")+g.family}),path1=Graph.MSAHMM.viterbi(hmm,seq1),eseq1=Graph.MSAHMM.sequenceEmissions(hmm,/*counts,*/seq1,path1);Graph.MSAHMM.alignToPath(genes1,path1);var genes2=JSON.parse(JSON.stringify(genes1)).reverse(),seq2=genes2.map(function(g){return(g.strand==-1?"+":"-")+g.family}),path2=Graph.MSAHMM.viterbi(hmm,seq2),eseq2=Graph.MSAHMM.sequenceEmissions(hmm,/*counts,*/seq2,path2);genes2.forEach(function(g){g.strand*=-1});Graph.MSAHMM.alignToPath(genes2,path2);if(path1.probability>=path2.probability){var oseq=Graph.MSAHMM.orientationSequence(eseq1,eseq2);rawGroups[i].genes=Graph.MSAHMM.orientatedGenes(genes1,genes2,oseq)}else{var oseq=Graph.MSAHMM.orientationSequence(eseq2,eseq1);rawGroups[i].genes=Graph.MSAHMM.orientatedGenes(genes2,genes1,oseq)}}return rawGroups};
