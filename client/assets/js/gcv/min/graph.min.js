...
this.addEdge("a","m0");this.addEdge("a","i0");this.addEdge("a","d0");for(var i=0;i<this.numColumns-1;i++){this.addEdge("m"+i,"m"+(i+1));this.addEdge("m"+i,"i"+(i+1));this.addEdge("m"+i,"d"+(i+1));this.addEdge("i"+i,"i"+i);this.addEdge("i"+i,"m"+i);this.addEdge("i"+i,"d"+i);this.addEdge("d"+i,"d"+(i+1));this.addEdge("d"+i,"m"+(i+1));this.addEdge("d"+i,"i"+(i+1))}this.addEdge("m"+(this.numColumns-1),"i"+this.numColumns);this.addEdge("m"+(this.numColumns-1),"z");this.addEdge("i"+this.numColumns,"i"+this.numColumns);this.addEdge("i"+this.numColumns,"z");this.addEdge("d"+(this.numColumns-1),"i"+this.numColumns);this.addEdge("d"+(this.numColumns-1),"z");this.updateTransitionProbabilities()}},{key:"updateTransitionProbabilities",value:function updateTransitionProbabilities(){for(var id in this.nodes){if(this.nodes.hasOwnProperty(id)){this.updateNodeTransitionProbabilities(id)}}}},{key:"updateNodeTransitionProbabilities",value:function updateNodeTransitionProbabilities(id){var pathsOnState=0;for(var pId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(pId)){pathsOnState+=this.nodes[id].attr.transitionCounts[pId]}}if(id=="d"+(this.numColumns-1)||id=="m"+(this.numColumns-1)||id=="i"+this.numColumns){for(var nId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(nId)){var pathsOnTransition=this.nodes[id].attr.transitionCounts[nId];var p=this.indelTransitionProbability(pathsOnState,pathsOnTransition);if(nId.startsWith("a")){p*=2}this.updateEdge(id,nId,p)}}}else{for(var nId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(nId)){var pathsOnTransition=this.nodes[id].attr.transitionCounts[nId];var p;if(nId.startsWith("m")){p=this.matchTransitionProbability(pathsOnState,pathsOnTransition)}else{p=this.indelTransitionProbability(pathsOnState,pathsOnTransition)}this.updateEdge(id,nId,p)}}}}}]);return _class8}(Graph.Directed),_class8.State=function(){function _class9(){_classCallCheck(this,_class9);this.transitionCounts={}}_createClass(_class9,[{key:"incrementTransition",value:function incrementTransition(id){if(!this.transitionCounts.hasOwnProperty(id)){this.transitionCounts[id]=0}this.transitionCounts[id]+=1}}]);return _class9}(),_temp3);Graph.MSAHMM.InsertState=function(_Graph$MSAHMM$State){_inherits(_class10,_Graph$MSAHMM$State);function _class10(){_classCallCheck(this,_class10);var _this9=_possibleConstructorReturn(this,(_class10.__proto__||Object.getPrototypeOf(_class10)).call(this));_this9.paths={};return _this9}_createClass(_class10,[{key:"addPath",value:function addPath(pId,o){if(!this.paths.hasOwnProperty(pId)){this.paths[pId]=[]}this.paths[pId].push(o)}}]);return _class10}(Graph.MSAHMM.State);Graph.MSAHMM.MatchState=function(_Graph$MSAHMM$State2){_inherits(_class11,_Graph$MSAHMM$State2);function _class11(characters){_classCallCheck(this,_class11);var _this10=_possibleConstructorReturn(this,(_class11.__proto__||Object.getPrototypeOf(_class11)).call(this));_this10.countAmplifier=1000;_this10.paths={};_this10.emissionCounts={};_this10.emissionProbabilities={};_this10.numObservations=characters.size;var p=1/_this10.numObservations;characters.forEach(function(o){_this10.emissionCounts[o]=1;// pseudo-count
_this10.emissionProbabilities[o]=p});return _this10}_createClass(_class11,[{key:"emit",value:function emit(o){return this.emissionProbabilities[o]}},{key:"addPath",value:function addPath(pId,o){this.paths[pId]=o;this.emissionCounts[o]+=this.countAmplifier;this.numObservations+=this.countAmplifier;for(var o in this.emissionProbabilities){if(this.emissionCounts.hasOwnProperty(o)){var p=this.emissionCounts[o]/this.numObservations;this.emissionProbabilities[o]=p}}}}]);return _class11}(Graph.MSAHMM.State);Graph.MSAHMM.viterbi=function(hmm,seq){var probs={},ptrs={};for(var id in hmm.nodes){if(hmm.nodes.hasOwnProperty(id)){probs[id]={};ptrs[id]={}}}// a generic probability forward propagate function
...
if(candidate>currentProb||candidate==currentProb&&currentPtr<from){probs[to][i]=candidate;ptrs[to][i]=from}};// recursively identifies the sequence's most probable path through the HMM
...
var e="z";propagate(dj,e,seq.length);propagate(ij,e,seq.length);propagate(mj,e,seq.length);// follow the pointers from the end state to the start state to get the path
return traceback(e,seq.length)};Graph.MSAHMM.embedPath=function(hmm,pId,path,seq){var i=0;for(var j=0;j<path.length-1;j++){var from=path[j],to=path[j+1],n=hmm.getNode(from).attr;if(n instanceof Graph.MSAHMM.InsertState||n instanceof Graph.MSAHMM.MatchState){n.addPath(pId,seq[i++])}n.incrementTransition(to);hmm.updateNodeTransitionProbabilities(from)}};/**
...
for(var i=0;i<groups.length;i++){console.log(groups[i].chromosome_name);// a) align to HMM
var seq=groups[i].genes.map(function(g){return g.family}),path=Graph.MSAHMM.viterbi(hmm,seq);console.log(seq);console.log(path);// b) embed alignment path and update transition and emission probabilities
Graph.MSAHMM.embedPath(hmm,i,path,seq);// c) if necessary, perform surgery on the graph
console.log(JSON.parse(JSON.stringify(hmm)))}console.log(hmm)};
