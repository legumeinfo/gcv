...
var l=groups[0].genes.length,families=new Set;for(var i=0;i<groups.length;i++){for(var j=0;j<groups[i].genes.length;j++){families.add(groups[i].genes[j].family)}}var hmm=new Graph.MSAHMM(l,families);// 2) iteratively train the HMM on the filtered tracks
...
for(var i=0;i<rawGroups.length;i++){var genes1=JSON.parse(JSON.stringify(rawGroups[i].genes)),seq1=rawGroups[i].genes.map(function(g){return g.family}),path1=Graph.MSAHMM.viterbi(hmm,seq1),eseq1=Graph.MSAHMM.sequenceEmissions(hmm,counts,seq1,path1);Graph.MSAHMM.alignToPath(genes1,path1);var genes2=JSON.parse(JSON.stringify(genes1)),seq2=seq1.slice().reverse(),path2=Graph.MSAHMM.viterbi(hmm,seq2),eseq2=Graph.MSAHMM.sequenceEmissions(hmm,counts,seq2,path2);genes2.reverse();genes2.map(function(g){g.strand*=-1});Graph.MSAHMM.alignToPath(genes2,path2);if(path1.probability>=path2.probability){var oseq=Graph.MSAHMM.orientationSequence(eseq1,eseq2);rawGroups[i].genes=Graph.MSAHMM.orientatedGenes(genes1,genes2,oseq)}else{var oseq=Graph.MSAHMM.orientationSequence(eseq2,eseq1);rawGroups[i].genes=Graph.MSAHMM.orientatedGenes(genes2,genes1,oseq)}}return rawGroups};
