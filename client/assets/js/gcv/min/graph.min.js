...
this.addEdge("a","m0");this.addEdge("a","i0");this.addEdge("a","d0");for(var i=0;i<this.numColumns-1;i++){this.addEdge("m"+i,"m"+(i+1));this.addEdge("m"+i,"i"+(i+1));this.addEdge("m"+i,"d"+(i+1));this.addEdge("i"+i,"i"+i);this.addEdge("i"+i,"m"+i);this.addEdge("i"+i,"d"+i);this.addEdge("d"+i,"d"+(i+1));this.addEdge("d"+i,"m"+(i+1));this.addEdge("d"+i,"i"+(i+1))}this.addEdge("m"+(this.numColumns-1),"i"+this.numColumns);this.addEdge("m"+(this.numColumns-1),"z");this.addEdge("i"+this.numColumns,"i"+this.numColumns);this.addEdge("i"+this.numColumns,"z");this.addEdge("d"+(this.numColumns-1),"i"+this.numColumns);this.addEdge("d"+(this.numColumns-1),"z");this.updateTransitionProbabilities()}},{key:"indelTransitionProbability",value:function indelTransitionProbability(pathsOnState,pathsOnTransition){//return (this.baseProbability * (pathsOnState + 1)) / (pathsOnTransition + 1);
return(1+pathsOnTransition)/(2+this.numCharacters+pathsOnState)}},{key:"matchTransitionProbability",value:function matchTransitionProbability(pathsOnState,pathsOnTransition){return(this.numCharacters+pathsOnTransition)/(2+this.numCharacters+pathsOnState)}},{key:"updateTransitionProbabilities",value:function updateTransitionProbabilities(){for(var id in this.nodes){if(this.nodes.hasOwnProperty(id)){this.updateNodeTransitionProbabilities(id)}}}},{key:"updateNodeTransitionProbabilities",value:function updateNodeTransitionProbabilities(id){var pathsOnState=0;for(var pId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(pId)){pathsOnState+=this.nodes[id].attr.transitionCounts[pId]}}if(id=="d"+(this.numColumns-1)||id=="m"+(this.numColumns-1)||id=="i"+this.numColumns){for(var nId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(nId)){var pathsOnTransition=this.nodes[id].attr.transitionCounts[nId];var p=(1+pathsOnTransition)/(2+pathsOnState);this.updateEdge(id,nId,p)}}}else{for(var nId in this.nodes[id].attr.transitionCounts){if(this.nodes[id].attr.transitionCounts.hasOwnProperty(nId)){var pathsOnTransition=this.nodes[id].attr.transitionCounts[nId];var p;if(nId.startsWith("m")){p=this.matchTransitionProbability(pathsOnState,pathsOnTransition)}else{p=this.indelTransitionProbability(pathsOnState,pathsOnTransition)}this.updateEdge(id,nId,p)}}}}}]);return _class8}(Graph.Directed),_class8.State=function(){function _class9(){_classCallCheck(this,_class9);this.transitionCounts={}}_createClass(_class9,[{key:"incrementTransition",value:function incrementTransition(id){this.transitionCounts[id]+=1}}]);return _class9}(),_temp3);Graph.MSAHMM.InsertState=function(_Graph$MSAHMM$State){_inherits(_class10,_Graph$MSAHMM$State);function _class10(){_classCallCheck(this,_class10);var _this9=_possibleConstructorReturn(this,(_class10.__proto__||Object.getPrototypeOf(_class10)).call(this));_this9.paths={};return _this9}_createClass(_class10,[{key:"addPath",value:function addPath(pId,o){if(!this.paths.hasOwnProperty(pId)){this.paths[pId]=[]}this.paths[pId].push(o)}}]);return _class10}(Graph.MSAHMM.State);Graph.MSAHMM.MatchState=function(_Graph$MSAHMM$State2){_inherits(_class11,_Graph$MSAHMM$State2);function _class11(characters){_classCallCheck(this,_class11);var _this10=_possibleConstructorReturn(this,(_class11.__proto__||Object.getPrototypeOf(_class11)).call(this));_this10.paths={};_this10.emissionCounts={};_this10.emissionProbabilities={};_this10.numObservations=characters.size;_this10.countAmplifier=_this10.numObservations;var p=1/_this10.numObservations;characters.forEach(function(o){_this10.emissionCounts[o]=1;// pseudo-count
...
console.log(JSON.parse(JSON.stringify(hmm)));for(var i=0;i<groups.length;i++){console.log(groups[i].chromosome_name);// a) align to HMM
...
if(path1.probability>=path2.probability){console.log("forward");console.log(seq1);console.log(path1);Graph.MSAHMM.embedPath(hmm,i,path1,seq1);align(path1,groups[i].genes)}else{console.log("reverse");console.log(seq2);console.log(path2);Graph.MSAHMM.embedPath(hmm,i,path2,seq2);groups[i].genes.reverse();align(path2,groups[i].genes)}// c) if necessary, perform surgery on the graph
console.log(JSON.parse(JSON.stringify(hmm)))}console.log(hmm);return groups};
