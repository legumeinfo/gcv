...
var traceback=function traceback(id,i){if(id=="s"){return[id]}var ptr=ptrs[id][i],path=traceback(ptr,i-!id.startsWith("d"));// arithmetic HACK!
...
return traceback(e,seq.length)};Graph.MSAHMM.embedPath=function(hmm,pId,seq,path){for(var i=0;i<path.length;i++){var n=hmm.getNode(path[i]).attr;if(n instanceof Graph.hmmInsert||n instanceof Graph.hmmMatch){n.addPath(pId,seq[i])}else{n.addPath(pId)}}};/**
  * An HMM based MSA algorithm.
  * @param {Array} tracks - groups attribute of GCV track data.
  * @return {int} - The computed score.
  */Graph.msa=function(groups){var performSurgery=function performSurgery(hmm){};// 1) construct the graph from the first track
...
var seq=groups[i].genes.map(function(g){return g.family}),path=Graph.MSAHMM.viterbi(hmm,seq);console.log(path);// b) embed alignment path and update transition and emission probabilities
