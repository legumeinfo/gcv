...
export class Plot extends Visualizer {
  ...
  protected draw() {
    ...
    this.decorateResize(() => {
      xAxis.resize();
      const y = this.viewer.attr("height") - this.bottom;
      xAxis.attr("transform", "translate(0, " + y + ")");
    });
    const xLabel = this.drawXLabel();
    this.bottom += (2 * this.PAD) + xLabel.node().getBBox().height;
    this.decorateResize(() => {
      xLabel.resize();
      const y = this.viewer.attr("height") -
                this.bottom +
    ...
    if (this.options.outlier !== undefined && this.data.genes.some((g) => {
      ...
      const outliers = this.drawOutliers();
      outliers.attr("x", this.left).attr("y", this.top);
      const h = outliers.node().getBBox().height;
    ...
    // draw the brush
    const clearButton = this.drawBrush(points.geneGroups, xAxis);
    this.bottom += clearButton.node().getBBox().height + this.PAD;
    this.decorateResize(() => {
      clearButton.resize();
      const y = this.viewer.attr("height") - this.PAD;
  ...
  private drawOutliers() {
    const outliers = this.viewer.append("text")
   ...
   */
  private drawXAxis() {
    const xCoords = this.data.genes.map((g) => g.x);
   ...
   */
  private drawYAxis() {
    const yCoords = this.data.genes.reduce((l, g) => {
          if (g.y !== this.options.outlier) {
            l.push(g.y);
          }
          return l;
        }, []);
    ...
    // draw the axis
    const yAxis = this.viewer.append("g").attr("class", "axis");
    // how the axis is resized
  ...
  private drawPoints() {
    const obj = this;
    const points = this.viewer.append("g");
    ...
    const genes = points.geneGroups.append("circle")
      .attr("r", this.RADIUS)
      .style("stroke", "#000")
      .style("cursor", "pointer")
      .attr("class", (e) => {
    ...
    const geneTips = points.geneGroups.append("text")
    ...
    points.resize = function(geneGroups) {
      geneGroups.attr("transform", (g) => {
        const x = this.xScale(g.x);
        const y = (g.y === this.options.outlier) ? this.outliers : this.yScale(g.y);
  ...
  private drawBrush(geneGroups, xAxis) {
    const obj = this;
    // the plot"s brush
    const clearButton = obj.viewer.append("text")
    ...
    let extent;
    const brush = d3.svg.brush().x(this.xScale).y(this.yScale)
        .on("brush", brushmove)
        .on("brushend", brushend);
    const brushG = this.viewer.append("g")
        .attr("class", "brush")
        .call(brush).on("click", () => {
    ...
    function brushmove() {
      ...
      brush.extent(extent);
      brushG.call(brush);
      geneGroups.classed("selected", (e) => {
        const isBrushed = extent[0][0] <= e.x && e.x <= extent[1][0];
        return isBrushed;
      });
    }
    function brushend() {
    ...
    function transitionData() {
      const domain = obj.xScale.domain();
      geneGroups.transition()
        .duration(500)
        .attr("transform", (g) => {
          const x = obj.xScale(g.x);
          const y = (g.y === obj.options.outlier) ? obj.outliers : obj.yScale(g.y);
          return "translate(" + x + ", " + y + ")";
        })
        .attr("visibility", (e) => {
