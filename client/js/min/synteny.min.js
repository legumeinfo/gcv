...
this.scale=d3.scale.linear().domain([0,data.length]).range([this.left,w-(this.right+this._PAD)]);// parse optional parameters
...
var yAxis=d3.svg.axis().orient("left").tickValues(ticks).tickFormat(function(y,i){return _this2.data.tracks[i].chromosome});// draw the axes of the graph
return this.viewer.append("g").call(yAxis).selectAll("text").style("cursor","pointer").on("mouseover",function(y,i){}).on("mouseout",function(y,i){}).on("click",this.options.nameClick)}/** Draws the viewer. */},{key:"_draw",value:function _draw(){// draw the x-axis
...
var ticks=[];for(var i=0;i<this.data.tracks.length;i++){// make the track
