...
var blocks=track.selectAll('block').data(t.blocks).enter().append('g').style('cursor','pointer').on('mouseover',function(b){obj._beginHover(d3.select(this))}).on('mouseout',function(b){obj._endHover(d3.select(this))}).on('click',this.options.blockClick);// help for generating points
...
.attr('data-y-top',yTop).attr('data-y-bottom',yBottom).attr('data-y-middle',yMiddle);return genPoints(b,yTop,yBottom,yMiddle)});// draw the tooltips
...
track.resize=function(polygons,tips){var obj=this;polygons.attr('points',function(b){var block=d3.select(this),yTop=block.attr('data-y-top'),yBottom=block.attr('data-y-bottom'),yMiddle=block.attr('data-y-middle');return genPoints(b,yTop,yBottom,yMiddle)});tips.attr('transform',function(b){var tip=d3.select(this),x=obj.scale(tip.attr('data-x')),y=tip.attr('data-y');return'translate('+x+', '+y+') '+tip.attr('data-rotate')})}.bind(this,polygons,tips);// how tips are rotated so they don't overflow the view
track.rotateTips=function(tips,resize){var vRect=obj.viewer.node().getBoundingClientRect();tips.classed('synteny-tip',false).attr('data-rotate',function(b){var tRect=this.getBoundingClientRect(),h=Math.sqrt(Math.pow(tRect.width,2)/2),// rotated height
r=tRect.bottom+h>vRect.bottom?45:-45;return'rotate('+r+')'}).classed('synteny-tip',true);resize()}.bind(this,tips,track.resize);return track}/**
...
tracks.forEach(function(t,i){t.rotateTips()});// move all tips to front
