...
export class Plot {
  ...
  private parseOptions(options): void {
    ...
    this.options.plotClick =
      this.options.plotClick ||
      ((plot) => {
        // noop
      });
    this.options.brushup =
      this.options.brushup ||
      ((brushed) => {
        // noop
      });
  ...
  private draw() {
    ...
    const brushended = (event) => {
      ...
      if (!s) {
        ...
        this.scale.x.domain(
          [s[0][0], s[1][0]].map(this.scale.x.invert, this.scale.x),
        );
        this.scale.y.domain(
          [s[1][1], s[0][1]].map(this.scale.y.invert, this.scale.y),
        );
