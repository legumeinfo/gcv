...
describe("parseParams", () => {
  ...
  it("coerces numeric alignment params, preserving negatives", () => {
    const raw = { match: "10", mismatch: "-1", gap: "-1", score: "30", threshold: "25" };
    expect(parseParams(raw, paramParsers)).toEqual({
      match: 10, mismatch: -1, gap: -1, score: 30, threshold: 25,
    });
  });

  it("splits a comma-separated sources string into an array", () => {
    expect(parseParams({ sources: "lis" }, paramParsers).sources).toEqual(["lis"]);
    expect(parseParams({ sources: "lis,phytozome" }, paramParsers).sources)
...
describe("param validation regexes", () => {
  ...
  it("POSITIVE_INT accepts positive integers only", () => {
    ["1", "10", "999"].forEach((v) => expect(accepts(Regex.POSITIVE_INT, v)).toBe(true));
