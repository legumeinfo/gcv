...
// api
import {
  MacroSyntenyBlocksComputeReply,
  MacroSyntenyBlocksComputeRequest,
  MacroSyntenyBlocksPromiseClient,
} from 'legumeinfo-microservices/dist/macrosyntenyblocks_service/v1';
...
export class PairwiseBlocksService extends HttpService {
  ...
  getPairwiseBlocks(
    ...
    if (request.type === GET || request.type === POST) {
      ...
      return this._makeHttpRequest<{ blocks: RawPairwiseBlocks[] }>(
        request,
        body,
      ).pipe(
        map((result) => {
  ...
  getPairwiseBlocksForTracks(
    ...
    return this._store.select(
      fromPairwiseBlocks.selectFilteredPairwiseBlocksForTracksAndTargets(
        tracks,
        sources,
        targets,
      ),
  ...
  getMacroColors(): Observable<Function | undefined> {
    ...
    if (macroConfig !== undefined && macroConfig.colors !== undefined) {
      ...
      return this._scriptService
        .loadScript(macroConfig.colors.scriptUrl)
        .pipe(endWith(func));
    }
