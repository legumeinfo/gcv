...
// store
import { getSelectedChromosomeIDs } from '@gcv/gene/store/selectors/chromosome/selected-chromosomes.selector';
...
export const getSelectedPartialBlockIDs = createSelectorFactory(memoizeArray)(
  ...
  (state: State, ids: TrackID[], sources: string[]): PairwiseBlocksID[] => {
    const reducer = (
      accumulator,
      { name: reference, source: referenceSource },
    ) => {
      sources.forEach((source) => {
        const partialID = pairwiseBlocksID(reference, referenceSource, source);
...
export const getSelectedPairwiseBlocks = createSelectorFactory(memoizeArray)(
  ...
  (state: State, ids: PairwiseBlocksID[]): PairwiseBlocks[] => {
    ...
    const reducer = (accumulator, id) => {
      const [reference, referenceSource, , chromosomeSource] = id.split(':');
      const partialID = pairwiseBlocksID(
        reference,
        referenceSource,
        chromosomeSource,
      );
