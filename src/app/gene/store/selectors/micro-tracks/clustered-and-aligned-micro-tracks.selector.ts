...
export const getClusteredSelectedMicroTracks = createSelectorFactory(
  memoizeArray,
)(
  ...
  (
    ...
    const clusters = clustering.hcluster(
      tracks,
      metric,
      params.linkage,
      params.cthreshold,
...
export const getClusteredAndAlignedSelectedMicroTracks = createSelectorFactory(
  memoizeObject,
)(
  getClusteredSelectedMicroTracks,
  (
          ...
          alignments.forEach((a) => {
            ...
            while (i < a.alignment.length) {
              if (a.orientations[i] == 1) {
                if (
                  j >= unalignedColumns.length ||
                  a.alignment[i] < unalignedColumns[j]
                ) {
                  a.alignment[i] -= j;
                  i += 1;
                } else {
...
export const getClusteredAndAlignedSearchMicroTracks = createSelectorFactory(
  memoizeArray,
)(
  ...
  (
    searchTracks: (Track & ClusterMixin)[],
    { consensuses, tracks },
...
export const getAllClusteredAndAlignedMicroTracks = createSelectorFactory(
  memoizeArray,
)(
  getClusteredAndAlignedSelectedMicroTracks,
  ...
  (
    ...
    const orderAlg =
      order in MICRO_ORDER_ALGORITHM_MAP
        ? (MICRO_ORDER_ALGORITHM_MAP[order].algorithm as (
            a: AlignmentMixin & ClusterMixin & Track,
            b: AlignmentMixin & ClusterMixin & Track,
          ) => number)
        : (t1, t2) => 0;
...
export const getAlignedMicroTrackCluster = (id: number) =>
  createSelectorFactory(memoizeArray)(
