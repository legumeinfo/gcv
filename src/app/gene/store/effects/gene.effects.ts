...
import { idArrayIntersection } from '@gcv/gene/store/reducers/gene.reducer';
...
export class GeneEffects {
  ...
  getSelected$ = createEffect(() => {
    return this._store.select(fromGene.getSelectedGeneIDs).pipe(
      ...
      concatMap(([{ action, names, source }, selectedIDs, loading]) => {
        ...
        return this.geneService.getGenes(filteredNames, source).pipe(
          ...
          switchMap((genes: Gene[]) => {
            ...
            const failedNames = filteredNames.filter(
              (n) => !successNames.has(n),
            );
  ...
  getMicroTrackGenes$ = createEffect(() => {
    return this._store
      .select(fromMicroTracks.getAllMicroTracks)
      .pipe(switchMap((tracks) => geneActions.tracksToGetGeneActions(tracks)));
  });
}
