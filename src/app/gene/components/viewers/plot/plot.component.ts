...
export class PlotComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  ngOnInit() {
    this.pipeline = {
      'Plot Genes': this._processService.getPlotGeneProcess(
        this.type,
        this.reference,
        this.track,
      ),
  ...
  ngAfterViewInit() {
    const plotFilter = (plot) => {
      return (
        plot.reference.name === this.reference.name &&
    ...
    const plot = plots.pipe(mergeAll(), filter(plotFilter));
    // NOTE: this feels awkward too...
    const genes = plot.pipe(
      switchMap((p) => {
        const sourceGeneMap = p.sourceGeneMap();
        const observables = Object.entries(sourceGeneMap).map(
          ([source, names]: [string, string[]]): Observable<Gene[]> => {
