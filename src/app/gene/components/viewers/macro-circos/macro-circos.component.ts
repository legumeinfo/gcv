...
export class MacroCircosComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  ngOnInit() {
    this.pipeline = {
      ...
      Positions: this._processService.getCircosBlockPositionProcess(
        this.clusterID,
      ),
    };
  }
  ...
  ngAfterViewInit() {
    const queryTracks = this._microTracksService.getSelectedClusterTracks(
      this.clusterID,
    );
    const queryChromosomes =
      this._chromosomeService.getSelectedChromosomesForCluster(this.clusterID);
    const colors = queryChromosomes.pipe(
    ...
    const blockGenes = combineLatest(
      ...
      map(([queries, chromosomes, blocks]) => {
        ...
        const geneChromosomes = chromosomes.map((c) => {
  ...
  private _preDraw(queries, chromosomes, blocks, genes, colors): void {
    const { data, highlight } = macroCircosShim(
      queries,
      chromosomes,
      blocks,
      genes,
    );
    const absolutePath = this._location.prepareExternalUrl(
      this._location.path(),
    );
