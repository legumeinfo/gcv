...
export function macroCircosShim(queries, chromosomes, pairwiseBlocks, genes) {
  ...
  const idFunction = format.includes(OrganismPlaceholders.Chromosome)
    ? chromosomeID
    : organismID;
  ...
  const data = Object.keys(chromosomesMap).map((referenceID) => {
    ...
    const referenceBlocks =
      referenceID in referenceMap ? referenceMap[referenceID] : [];
    const chromosomeTracks = macroBlocks(
      referenceTrack,
      referenceBlocks,
      genesMap,
    );
