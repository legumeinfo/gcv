...
function _greedyIntervalScheduling<T>(
  intervals: ({ begin: number; end: number } & T)[],
...
function _tracksToData(tracks, genes, omittedFamilies) {
  ...
  return tracks.map((t, j) => {
    ...
    const forwardSegments = t.segments.map((s, i) =>
      t.orientations[i] == 1 ? s : null,
    );
    ...
    const reverseSegments = t.segments.map((s, i) =>
      t.orientations[i] == -1 ? s : null,
    );
    ...
    const reverseIntervalObjects = reverseIntervals
      ...
      .sort((o1, o2) => o1.end - o2.end);
    const reverseSchedules = _greedyIntervalScheduling(
      reverseIntervalObjects,
    ).map((schedule) => schedule.reverse());

    const schedules = forwardSchedules
      .concat(reverseSchedules)
