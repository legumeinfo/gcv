...
export class GeneComponent implements AfterViewInit, OnDestroy {
  ...
  private _addPlots(id, track, queryTracks, type): void {
    const plotStackConfig = this._stackItem(
      id,
      fromViewers.plotStackConfigFactory,
      track,
    );
    ...
    const geneOver = (e, gene, family, source) => {
      ...
      const config = fromTooltips.geneTooltipConfigFactory(
        inputs,
        this._tipOptions,
      );
  ...
  private _addMicroViewers(clusterIDs): void {
    const options = {
      plotClick: (e, id, track, queryTracks) => {
        ...
        const config = fromTooltips.plotTooltipConfigFactory(
          outputs,
          this._tipOptions,
        );
        this.tooltipFactoryDirective.componentTip(e.target, config);
      ...
      geneClick: (id, gene, family, source) => {
        this._stackItem(
          id,
          fromDetails.geneDetailConfigFactory,
          gene,
          family,
          source,
        );
      ...
      reference: (id, name, source, clusterID) => {
        const outputs = {
          blockOver: (e, pairwiseBlocks, block) => {
            ...
            const config = fromTooltips.macroBlockTooltipConfigFactory(
              inputs,
              this._tipOptions,
            );
            this.tooltipFactoryDirective.componentTip(e.target, config);
