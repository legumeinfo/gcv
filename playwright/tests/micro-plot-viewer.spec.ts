...
const REPRO =
  '/gene;lis=cicar.CDCFrontier.gnm1.ann1.Ca_21954' +
...
test.describe('micro-synteny plot viewer (Golden Layout v2)', () => {
  ...
  test('opening a plot does not throw SACC88532 and renders a plot viewer', async ({
    page,
  }) => {
    const errors: string[] = [];
    page.on('console', (m) => {
  ...
  test('local and global plots share a single pane as two tabs', async ({
    page,
  }) => {
    await openPlot(page, 'local');
