...
for(var j=0;j<groupTracks.length&&merged.indexOf(j)==-1;j++){var jTrack=groupTracks[j],jLevels=0,jIds=jTrack.genes.map(function(g){return g.id});for(var k=j+1;k<groupTracks.length&&merged.indexOf(k)==-1;k++){var kTrack=groupTracks[k],kLevels=0,kIds=kTrack.genes.map(function(g){return g.id});// compute the intersection
...
var minX=Infinity,maxX=-Infinity;var minDistance=Infinity,maxDistance=-Infinity;this.names=[];this.ticks=[];var tick=0;this.distances=[];for(var i=0;i<this.data.groups.length;i++){var group=this.data.groups[i],fminI=Infinity,fmaxI=-Infinity;this.ticks.push(tick);tick+=levels[i];var distances=[];for(var j=0;j<group.genes.length;j++){var gene=group.genes[j],fmax=gene.fmax,fmin=gene.fmin,x=gene.x;fminI=Math.min.apply(null,[fminI,fmax,fmin]);fmaxI=Math.max.apply(null,[fmaxI,fmax,fmin]);minX=Math.min(minX,x);maxX=Math.max(maxX,x);if(j<group.genes.length-1){var nextGene=group.genes[j+1],nextFmin=nextGene.fmin,nextFmax=nextGene.fmax;var dist=Math.min.apply(null,[fmin-nextFmin,fmax-nextFmin,fmin-nextFmax,fmax-nextFmin]);distances.push(dist);minDistance=Math.min(minDistance,dist);maxDistance=Math.max(maxDistance,dist)}}this.names.push(group.chromosome_name+':'+fminI+'-'+fmaxI);this.distances.push(distances)}// initialize the x, y, and line thickness scales
...
var tips=track.selectAll('.synteny-tip');track.rotateTips=function(tips,resize){var vRect=obj.viewer.node().getBoundingClientRect();tips.classed('synteny-tip',false).attr('transform',function(t){var tRect=this.getBoundingClientRect(),h=Math.sqrt(Math.pow(tRect.width,2)/2),// rotated height
