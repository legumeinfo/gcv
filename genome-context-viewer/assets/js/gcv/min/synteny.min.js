...
for(var i=orderedBlocks.length-1;i>=0;i--){if(orderedBlocks[i].start>row[k].stop){orderedBlocks[i].y=y;row.push.apply(row,orderedBlocks.splice(i,1));k++}}rows.push(row)}}/**
    ...
    */},{key:'_drawTrack',value:function _drawTrack(i){var _this3=this;var obj=this,c=this._COLORS[i%this._COLORS.length],t=this.data.tracks[i];// create the track's rows of blocks
...
for(var i=0;i<sheets.length;i++){var rules=sheets[i].rules||sheets[i].cssRules;for(var r in rules){var rule=rules[r],selector=rule.selectorText;if(selector!==undefined&&selector.startsWith('.GCV')){var style=rule.style,selection=clone.selectAll(selector);for(var k=0;k<style.length;k++){var prop=style[k];selection.style(prop,style[prop])}}}}return clone}/** Saves the viewer to as an SVG. */},{key:'save',value:function save(){try{var isFileSaverSupported=!!new Blob}catch(e){alert('Your broswer does not support saving')}// create a clone of the viewer with all GCV styles inlined
var clone=this._inlineCopy();// generate the data
var xml=new XMLSerializer().serializeToString(clone.node());var blob=new Blob([xml],{type:'image/svg+xml'});// save the data
var url=window.URL.createObjectURL(blob);var a=document.createElement('a');a.style='display: none';a.href=url;var date=new Date;a.download='gcv-macro-synteny-'+date.toISOString()+'.svg';document.body.appendChild(a);a.click();window.URL.revokeObjectURL(url);document.body.removeChild(a)}/** Manually destroys the viewer. */},{key:'destroy',value:function destroy(){if(this.container){this.container.removeChild(this.viewer.node());if(this.resizer){this.container.removeChild(this.resizer)}}this.container=this.viewer=this.resizer=undefined}}]);return _class2}();
