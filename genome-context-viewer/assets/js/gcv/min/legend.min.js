...
var rect=key.append('rect').attr('width',this._RECT_SIZE).attr('height',this._RECT_SIZE).style('fill',function(){return _this2.colors(f.id)});// add then labels
...
var presentFamilies=this.data.groups.reduce(function(l,group){return l.concat(group.genes.map(function(g){return g.family}))},[]);var families=this.data.families.filter(function(f){return presentFamilies.indexOf(f.id)!=-1&&!(f.name==''||_this3.options.selectiveColoring!==undefined&&_this3.options.selectiveColoring[f.id]==1)});legend.keys=[];families.forEach(function(f,i){var k=_this3._drawKey(legend,f),y=legend.node().getBBox().height;if(i>0)y+=2*_this3._PAD;k.attr('transform','translate(0, '+y+')');legend.keys.push(k)});// implement the resize function
...
var url=window.URL.createObjectURL(blob);var a=document.createElement('a');a.style='display: none';a.href=url;var date=new Date;a.download='gcv-legend-'+date.toISOString()+'.svg';document.body.appendChild(a);a.click();window.URL.revokeObjectURL(url);document.body.removeChild(a)}/** Manually destroys the viewer. */},{key:'destroy',value:function destroy(){if(this.resizer){if(this.resizer.contentWindow)this.resizer.contentWindow.onresize=undefined;this.container.removeChild(this.resizer)}this.container.removeChild(this.viewer.node());this.container=this.viewer=this.resizer=undefined}}]);return _class2}();
